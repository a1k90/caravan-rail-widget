...
# -*- coding: utf-8 -*-
"""
Caravan Railroad - Full v3.6.0 Upgrade
"""

import json, re, sqlite3, os

def run_upgrade():
    print("=== Step 1: Loading Stations & Cargo from DB ===")
    conn = sqlite3.connect("caravan_logistics.db")
    cur = conn.cursor()
    
    cur.execute("""
    SELECT code, name, country, admin, road, road_name, is_border, country_name
    FROM stations
    WHERE is_border = 1 
       OR name IN (
          "Кокшетау", "Астана", "Караганда", "Шымкент", "Алматы 1", "Алматы 2", "Актобе", "Атырау", "Мангышлак", "Павлодар", "Костанай", "Семей", "Усть-Каменогорск", "Тараз", "Кызылорда",
          "Ташкент-Товарный", "Чукурсай", "Сергели", "Самарканд", "Бухара 1", "Бухара 2", "Навои", "Карши", "Термез", "Андижан 1", "Фергана 1", "Коканд 1", "Ургенч", "Нукус", "Джизак", "Ангрен",
          "Москва-Товарная-Павелецкая", "Санкт-Петербург-Товарный-Витебский", "Самара", "Саратов 1", "Екатеринбург-Товарный", "Челябинск-Главный", "Новосибирск-Главный", "Омск-Пассажирский", "Красноярск", "Иркутск-Пассажирский", "Ростов-Товарный", "Волгоград 1", "Нижний Новгород-Сортировочный", "Казань", "Уфа", "Пермь 2", "Владивосток", "Хабаровск 1",
          "Минск-Товарный", "Брест-Центральный", "Гомель", "Витебск", "Могилев 1", "Барановичи-Центральные",
          "Бишкек 1", "Бишкек 2", "Ош", "Джалал-Абад", "Рыбачье",
          "Душанбе 1", "Душанбе 2", "Худжанд", "Курган-Тюбе",
          "Ашхабад", "Туркменбаши 1", "Туркменабад 1", "Мары",
          "Баку-Товарный", "Гянджа", "Алят", "Ялама", "Беюк-Кясик",
          "Тбилиси-Товарная", "Поти", "Батуми",
          "Улан-Батор", "Сухэ-Батор", "Дзамын-Уудэ"
       )
       OR code IN ("687008", "720000", "720104", "704101", "708507", "707701", "666501", "664900", "816909", "815502", "711105", "688708", "843905", "662905", "734606", "736501", "736003", "663607", "706304", "000251", "663404", "693602")
    ORDER BY is_border DESC, name ASC
    """)
    
    stn_rows = cur.fetchall()
    stations = []
    seen = set()
    for code, name, country, admin, road, road_name, is_border, country_name in stn_rows:
        if code in seen: continue
        seen.add(code)
        stations.append({
            "code": code,
            "name": name,
            "country": country or "KAZ",
            "country_name": country_name or "",
            "admin": admin or "КТЖ",
            "road": road or "67",
            "road_label": road_name or "Казахстанская ж. д. (КТЖ)",
    ...
    print("=== Step 2: Generating Clean Engine Code ===")
...
var CaravanRailwayEngine = (function() {{
  ...
  function calculateTariff(params) {{
    ...
    return {{
      ...
      incoterms: incoterms,
      totalKm: totalKm,
      transitDaysStr: transitDaysMin + '-' + transitDaysMax + ' суток',
...
"""
    ...
    with open("railway_calc_engine.js", "w", encoding="utf-8") as f:
        f.write(engine_js)
    print("railway_calc_engine.js written successfully!")
    ...
    # 2. Replace input styles: dark blue, right-aligned icons, proper padding
    old_input_css = re.search(r'\.cr-input-wrapper\s*\{.*?\.cr-clear-btn\s*\{', content, re.DOTALL)
    if old_input_css:
        new_input_css = """.cr-input-wrapper {
    position: relative;
    width: 100%;
    display: flex;
    align-items: center;
  ...
  .cr-clear-btn {"""
        content = content[:old_input_css.start()] + new_input_css + content[old_input_css.end() - len(".cr-clear-btn {"):]

    # 3. Add styling for .cr-mod-svg
    if ".cr-mod-svg" not in content:
        content = content.replace(".cr-mod-icon {", """.cr-mod-icon {
    display: inline-flex;
    align-items: center;
  ...
  .cr-mod-icon_old {""")
    ...
    # 4. Replace Engine cleanly using verified boundary slice
    s_idx = content.find("var CaravanRailwayEngine = (function() {")
    e_idx = content.find("(function() {\n  var CARAVAN_CONFIG = {")
    if s_idx != -1 and e_idx != -1:
        content = content[:s_idx] + engine_js.strip() + "\n\n" + content[e_idx:]
        print("Replaced CaravanRailwayEngine in caravan-tracking-widget.html cleanly!")
    else:
        print("ERROR: Could not find exact engine boundary in caravan-tracking-widget.html!")
        return

    with open("caravan-tracking-widget.html", "w", encoding="utf-8") as f:
        f.write(content)

    print("=== Step 4: Updating server/server.py ===")
    ...
    if '("710007", "704101")' not in srv_code:
        ...
        with open("server/server.py", "w", encoding="utf-8") as f:
            f.write(srv_code)
        print("Updated CANONICAL_CORRIDORS in server/server.py!")

    print("=== Step 5: Mirroring to index.html and Rebuilding Bundles ===")
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(content)

    # Rebuild bundle
    from build_widget_bundle import build
    build()

    # Update snippet version in tilda-embed-snippet.html
    with open("tilda-embed-snippet.html", "r", encoding="utf-8") as f:
        snippet = f.read()
    snippet = snippet.replace("v=3.5.0", "v=3.6.0")
