...
(function() {
  ...
  function setupCalculator() {
    ...
    // Слушатель разворачивания/сворачивания маршрутного листа — только через window.toggleItineraryList (onclick)
    // Дублирующий addEventListener намеренно убран: одновременный вызов onclick + listener флипал состояние дважды
    ...
    input.addEventListener('input', function() {
      ...
      if (val.length < 2) {
        ...
        return;
            ...
            function sanitizeStationObj(st) {
        if (!st) return st;
  ...
  window.toggleItineraryList = function(e) {
    if (e && e.preventDefault) e.preventDefault();
    var content = document.getElementById('cr-itinerary-content');
    var btnText = document.getElementById('cr-itin-btn-text');
    var chevron = document.getElementById('cr-itin-chevron');
    if (!content) return;
    if (content.style.display === 'none' || !content.style.display) {
      content.style.display = 'block';
      if (btnText) btnText.textContent = 'Свернуть список станций';
      if (chevron) chevron.style.transform = 'rotate(180deg)';
    } else {
      content.style.display = 'none';
      if (btnText) btnText.textContent = 'Развернуть список станций';
      if (chevron) chevron.style.transform = 'rotate(0deg)';
    }
  };

  function renderRouteItinerary(itinerary) {
    var card = document.getElementById('cr-itinerary-card');
    var tbody = document.getElementById('cr-itinerary-tbody');
    var countEl = document.getElementById('cr-itin-count');
    var distEl = document.getElementById('cr-itin-dist');
    ...
    var totalDist = itinerary[itinerary.length - 1].cumulativeKm || 0;
    if (countEl) countEl.textContent = itinerary.length;
    if (distEl) distEl.textContent = totalDist.toLocaleString('ru-RU');

    var html = '';
    itinerary.forEach(function(st, idx) {
      var rowClass = '';
      var badgeHtml = '';
      if (st.isOrigin) {
        rowClass = 'origin-row';
        badgeHtml = '<span class="cr-itinerary-badge origin">Отправление</span> ';
      } else if (st.isDestination) {
        rowClass = 'dest-row';
        badgeHtml = '<span class="cr-itinerary-badge dest">Назначение</span> ';
      } else if (st.isBorder) {
        rowClass = 'border-row';
        badgeHtml = '<span class="cr-itinerary-badge border">Стык ' + escapeHtml(st.borderLabel || '') + '</span> ';
      }

      var roadStr = (st.road || '') + (st.countryName ? ' (' + st.countryName + ')' : '');

      html += '<tr class="' + rowClass + '">' +
        '<td style="color: #64748B;">' + (idx + 1) + '</td>' +
        '<td>' + badgeHtml + '<strong>' + escapeHtml(st.name) + '</strong></td>' +
        '<td style="font-family: monospace; color: #94A3B8;">' + escapeHtml(st.code) + '</td>' +
        '<td>' + escapeHtml(roadStr) + '</td>' +
        '<td class="cr-text-right">' + (st.segmentKm > 0 ? '+' + st.segmentKm.toLocaleString('ru-RU') + ' км' : '—') + '</td>' +
        '<td class="cr-text-right cr-cum-km">' + st.cumulativeKm.toLocaleString('ru-RU') + ' км</td>' +
      '</tr>';
    });
